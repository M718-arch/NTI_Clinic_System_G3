...
export const Messages = ({ updateUnreadCount }) => {
    ...
    const startNewConversation = async (patient) => {
        ...
        try {
            ...
            notify('error', serverMessage || 'Could not start the conversation. Please try again.');
        }
    };
                ...
                <div className={`w-full md:w-80 shrink-0 border-r border-slate-200 overflow-auto bg-white ${showChatOnMobile ? 'hidden md:block' : 'block'}`}>
                    {filteredConversations.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-slate-400 p-6 text-center">
                            ...
                            <p className="text-xs text-slate-400 mt-1">Message one of your patients to get started.</p>
                            <button
                                onClick={() => setShowNewChat(true)}
                        ...
                        <div ref={chatContainerRef} className="flex-1 overflow-auto px-4 md:px-5 py-4 bg-slate-50/40">
                            {messages.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center">
                                    ...
                                    <p className="text-xs">Say hello to {activeConversation.other_user?.name}</p>
                                </div>
                            ) : (
            ...
            {showNewChat && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full max-h-[80vh] flex flex-col">
                        ...
                        <p className="text-xs text-slate-400 mb-4">Only patients who have booked an appointment with you appear here.</p>
