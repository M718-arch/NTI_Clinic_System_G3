// resources/js/components/Doctor/pages/PatientList.jsx

import React, { useState, useEffect } from 'react';
import { Search, MoreVertical, X, Plus, TrendingUp } from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';
import api from '../../../api/client';
import DoctorEmrForm from "./DoctorEmrForm";
...
export const PatientList = ({ onSelectPatient }) => {
    ...
    const [activeTab, setActiveTab] = useState('details'); // 'details', 'emr', 'prescriptions'
    ...
    const handleEmrSuccess = () => {
        setMessage({ type: 'success', text: 'EMR record added successfully' });
        setTimeout(() => setMessage({ type: '', text: '' }), 3000);
    };
    ...
    // Render Phase 8 EMR/Patient Content in Modal
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 bg-[#f5f6ff] text-[#252f43] font-body">
                                        ...
                                        <tr key={patient.id} className="hover:bg-white/30 transition-colors group animate-fade-in-up" style={{ animationDelay: `${0.3 + index * 0.05}s` }}>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    {/* ✅ Open full EMR chart (DoctorPatientChart via dashboard) */}
                                                    <button 
                                                        onClick={() => onSelectPatient?.(patient)}
                                                        className="p-2 text-[#525b72] hover:text-[#166534] transition-colors rounded-full hover:bg-white/50"
                                                        title="View Full Chart"
                                                    >
                                                        <span className="material-symbols-outlined text-[18px]">folder_open</span>
                                                    </button>
                                                    {/* ✅ Phase 8: EMR Button (opens local quick-add modal) */}
                                                    <button 
                                                        onClick={() => handlePatientAction(patient, 'emr')}
                                                        className="p-2 text-[#525b72] hover:text-[#6f4b94] transition-colors rounded-full hover:bg-white/50"
                                                        title="Add EMR Record"
                                                    >
                                                        <span className="material-symbols-outlined text-[18px]">clinical_notes</span>
                                                    </button>
                                                    {/* ✅ Phase 8: Prescription Button */}
                                                    <button 
                                                    ...
                                                    {/* Existing Details Button (local modal) */}
                                                    <button 
            ...
            {/* Patient Detail Modal with Phase 8 Tabs */}
            {showModal && selectedPatient && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-[4px] flex items-center justify-center z-50 p-4 animate-fade-in-up">
                    <div className="glass-elevated rounded-2xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                        {/* Modal Header */}
                        <div className="flex justify-between items-center mb-4 sticky top-0 bg-white/50 backdrop-blur-sm py-3 -mt-3 px-1 rounded-t-xl z-10">
                            <div className="flex items-center gap-3">
                                <PatientAvatar patient={selectedPatient} sizeClass="w-10 h-10" textClass="text-sm" />
                                <div>
                                    <h3 className="text-lg font-headline font-semibold text-[#252f43]">{selectedPatient.name}</h3>
                                    <p className="text-sm text-[#525b72]">Patient ID: {selectedPatient.id}</p>
                                </div>
                            </div>
                            <button
                                onClick={() => {
                        ...
                        <div className="flex gap-2 border-b border-white/20 mb-4">
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-medium transition ${
                                    activeTab === 'details'
                                        ? 'text-[#006382] border-b-2 border-[#006382]'
